...
def build_stage_prompt(
    ...
    if stage == "analyzer_report":
        return (
            ...
            "Your job is to understand the post, identify the main claim, and explain what kind of skill would help most.\n"
            "Do not give the final fake/real verdict yet.\n\n"
            + shared
            + "Write exactly three plain-text lines in this format:\n"
            + "Visual: what is visibly shown in the frames.\n"
            + "Claim: the main concrete factual claim the post makes.\n"
            + "Need: what verification skill or principle would help judge this case correctly.\n"
    ...
    if stage == "worker_skill":
        return (
            ...
            "Read the Analyzer report, inspect the retrieved skills below, and return one short skill the Analyzer should use.\n"
            ...
            + "Focus on the rule that will help the Analyzer avoid the main reasoning mistake in this case.\n"
        )

    if stage == "verdict":
        return (
            "You are Analyzer.\n"
            "Use the original post plus the Worker-provided skill to decide whether the post is fake or real.\n"
